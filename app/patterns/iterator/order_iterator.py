...
from app.patterns.composite.order_composite import OrderComponent, OrderGroup, OrderLeaf  # noqa: F401
...
class CargoTypeFilterIterator:
    """
    Декорирующий итератор: фильтрует листья по типу груза (document / package).
    Компонуется с FilterIterator для двойной фильтрации.
    """

    def __init__(self, iterator: OrderIterator, cargo_type: str):
        self._items = [
            leaf for leaf in iterator
            if leaf.order.cargo_type == cargo_type
        ]
        self._index = 0
