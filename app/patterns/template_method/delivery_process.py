...
class BaseDeliveryProcess(ABC):
    ...
    def calculate_price(self, order):
        """Конкретный метод - расчет цены по стратегии"""
        tariff_map = {
            'economy': EconomyTariff(),
            'standard': StandardTariff(),
            'express': ExpressTariff(),
        }
        strategy = tariff_map.get(order.tariff, StandardTariff())
        order.price = strategy.calculate_price(5.0)
        order.save()
        self.logger.log(f"Цена: {order.price} руб.")
